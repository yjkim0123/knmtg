...
urban metro stations, 47 high-speed and express metro terminals,
254 regional rail stops, 42 intercity bus terminals, and 3 BRT
hubs---connected by 1,458 directed edges with empirically calibrated
travel times and transfer penalties.
...
all five modes shows a mean absolute error of 2.4~minutes relative to
...
\citet{derrible2009} analyzed the topological
...
The five highest-degree nodes are Express Bus Terminal/Gosok (41),
Gangnam (22), Cheonan (22), Suwon (21), and Iksan KTX (20).
Express Bus Terminal's exceptional degree reflects its role as the
primary Seoul intercity bus terminus, connected to numerous
out-of-Seoul intercity routes.

\paragraph{Betweenness centrality.}
Approximate betweenness centrality (sampled from 300 source nodes)
identifies Suseo/Suseo-SRT (BC=67,603), Busan KTX (60,906), and
Seoul Station (60,171) as the highest-betweenness nodes---consistent
with their role as national transit bottlenecks on the Gyeongbu KTX
corridor. The prominence of Dongtan SRT (BC=47,805) reflects the
SRT's role in connecting the southern metropolitan area with the
national network.
...
To reduce the candidate set from all 932 nodes, stations outside the
...
$|V|=932$, $|E|=1{,}458$, a query completes in under 50 ms in a Python
...
\small
\begin{tabular}{p{4.5cm}lrrr}
\toprule
Route & Mode & KNMTG & Ref. & Err. \\
      &      & (min) & (min) & (min) \\
\midrule
Seoul KTX $\to$ Busan KTX & KTX & 141 & $\sim$150 & $-9$ \\
Seoul KTX $\to$ Daejeon KTX & KTX & 56 & $\sim$48 & $+8$ \\
Seoul KTX $\to$ Gwangju-Songjeong & KTX & 110 & $\sim$88--110 & $\pm 0$ \\
Seoul KTX $\to$ Dongdaegu KTX & KTX & 101 & $\sim$95 & $+6$ \\
Suwon $\to$ Seoul Station & Saemaul & 22 & $\sim$22 & $0$ \\
Banjuk $\to$ Daejeon (metro) & Metro & 42 & $\sim$41 & $+1$ \\
Gangnam $\to$ Hongik Univ. & Metro & 39 & $\sim$30--40 & $\pm 0$ \\
Jamsil $\to$ Gangnam & Metro & 14 & $\sim$12--14 & $0$ \\
Daejeon KTX $\to$ Daejeon (metro) & Transfer & 10 & $\sim$10 & $0$ \\
Daejeon Bus Term. $\to$ Daejeon (metro) & Bus link & 15 & $\sim$15 & $0$ \\
Osong Bus Hub $\to$ Jochiwon & City bus & 5 & $\sim$5 & $0$ \\
Dongtan BRT Hub $\to$ Suwon & BRT & 25 & $\sim$30 & $-5$ \\
\midrule
\multicolumn{3}{l}{Mean Absolute Error} & & \textbf{2.4} \\
\multicolumn{3}{l}{RMSE} & & \textbf{4.2} \\
\bottomrule
\end{tabular}
\end{table}

The mean absolute error is 2.4 minutes across validated routes.
Larger errors occur for KTX routes where we use average rather than
fastest-service times. Two known limitations are acknowledged:
(1) Busan Station (subway) is not yet co-located with the Busan KTX
node, leaving the Busan metro Line 1 partially disconnected from
the KTX terminal; (2) Dongdaegu Station (Korail) lacks a direct
transfer link to Daegu urban metro, requiring future addition of a
cross-mode transfer edge.
...
travel times with a mean absolute error of 2.4 minutes. Building on
